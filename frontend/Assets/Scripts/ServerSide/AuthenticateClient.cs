...
public class AuthenticateClient : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        GameManager.Instance.GetGameData().gameConfig = new GameConfig();
        GameManager.Instance.GetGameData().gameConfig.apiBaseUrl = "http://localhost:8000/";
        GameManager.Instance.GetGameData().gameConfig.authPhpEndpoint = "src/game_client_login.php";
        GameManager.Instance.GetGameData().gameConfig.authPythonEndpoint = "gate_data";
        Debug.Log("python endpoint: " + GameManager.Instance.GetGameData().gameConfig.authPythonEndpoint);
    }
    ...
    IEnumerator AuthClient(string username, string password)
    {
        ...
        string url = GameManager.Instance.GetGameData().gameConfig.apiBaseUrl + GameManager.Instance.GetGameData().gameConfig.authPhpEndpoint;
    ...
    private void ParseJsonData(string jwt, string json)
    {
        ...
        StartCoroutine(HandlePayload(jwt, payload));
    }


    IEnumerator HandlePayload(string jwt, string payload)
    {
        ...
        string url = "http://localhost:5000/gate_data";
        UnityWebRequest request = new UnityWebRequest(url, "POST");
